...
\begin{center}
	...
	\begin{tikzpicture}[node distance = 3cm, auto, thick,scale=0.3, every node/.style={transform shape}]
		...
		\node at (2,0.5) {Si (p-type)};
		% phosphorus
		\fill[Goldenrod] (0.25,1.9) rectangle (7.25,2.1);
		\node at (4.75,1.9) {n+};
		% oxide
		\fill[gray] (0,2) rectangle (0.25,2.6);
		\fill[gray] (7.25,2) rectangle (15,2.6);
...
\begin{center}
	...
	\end{tikzpicture}
\end{center}
First we use hydrofluoric acid, because it doesn't etch silicon at all but is very aggressive towards $SiO_2$
...
\begin{center}
	...
	\end{tikzpicture}
\end{center}
In this step we fill the hole we had opened in order to form the n-well.
