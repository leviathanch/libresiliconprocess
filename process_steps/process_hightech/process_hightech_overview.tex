...
\begin{figure}[H]
	...
	\begin{tikzpicture}[node distance=2cm, thick,scale=0.8, every node/.style={transform shape}]
		...
		%active CMOS	
		% first row
		\node [block] (nwell) at (0,8) {N-Well\\ \autoref{nwell_chapter}};
		\node [block, above of=nwell] (pwell) {P-Well\\ \autoref{pwell_chapter}};
		\node [block, above of=pwell] (pbase) {P-Base\\ \autoref{pbase_chapter}};
		\node [block, above of=pbase] (nbase) {N-Base\\ \autoref{nbase_chapter}};
		\node [block, above of=nbase] (sti) {Isolation (STI)\\ \autoref{sti_chapter}};
