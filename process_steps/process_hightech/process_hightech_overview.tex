...
\begin{figure}[H]
	...
	\begin{tikzpicture}[node distance=2cm, thick,scale=0.7, every node/.style={transform shape}]
		...
		\node [block, right=0.75cm of pp] (silicification){Silicification\\ \autoref{step_silicification}};

		\node [block, right=0.75cm of silicification] (contact) {Contact \\ \autoref{contact}};
		\node [block, below of=contact] (metal1) {Metal 1\\ \autoref{metal}};
		\node [block, below of=metal1] (via1) {Via 1\\ \autoref{via}};
		\node [block, below of=via1] (metal2) {Metal 2\\ \autoref{more_metal}};
		\node [block, below of=metal2] (via2) {Via 2\\ \autoref{via}};
		\node [block, below of=via2] (metal3) {Metal 3\\ \autoref{more_metal}};
		\node [block, below of=metal3] (glass) {Glass\\ \autoref{via}};
