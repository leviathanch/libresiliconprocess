...
After these steps we have to remove the nitride mask and even out the excess oxide for which polishing is the perfect method because it removes the nitride and the excess oxide at the same time.
...
The thin layer of "pad" oxide (around 300nm) is grown in dry ambient for 45 minutes at 1000\degreesC\footnote{\url{http://cleanroom.byu.edu/OxideTimeCalc}} in the diffusion furnace (\autoref{diffusion_furnace_machine}).
...
The LPCVD machine from HKUST(\autoref{lpcvd_machine}) is being used with the recipe:
\begin{itemize}
	\item Temperature: 810\degreesC  Flat from (door to pump)
	\item Pressure = 400 mTorr ($3 Si H_2 Cl_2 + 4 N H_3 = Si_3 N_4 + 9 H_2 + 3 Cl_2$
	\item Dichlorsilane ($Si H_2 Cl_2$) Flow=60sccm
	\item Ammonia ($N H_3$) flow = 150sccm
\end{itemize}
The resulting growth rate is around 6nm $\pm 1nm$.

The required thickness of this layer is not that critical, it can very well variate between 6nm and 10nm.\footnote{\url{https://www.google.com/patents/US7985656}}, for this reason we can put it into the LPCVD for around one or two minutes with the above recipe.
